/************************************************************************/
/*  Archivo:                trn_cj_reest.sp                             */
/*  Stored procedure:       sp_trn_cj_reest                             */
...
/*  Generacion de archivo con una dependecia de                         */
/*  cob_interface..sp_genera_orden                                      */
/*                                                                      */
/************************************************************************/
/*                         MODIFICACIONES                               */
/*  FECHA             AUTOR            RAZON                            */
...
if exists(select 1 from sysobjects where name ='sp_trn_cj_reest')
    drop proc sp_trn_cj_reest
go

create proc sp_trn_cj_reest(
...
begin
   ...
   begin
      ---Sacar el maximo tramite primero porque pueden haber varias Reestructuraciones para el mismo i_banco
...
--------------------------------------------------------------------------------------------------
--OPERACION 'I'--> INSERTA LA TRANSACCION DE ABONO PARA REESTRUCTURACION
--------------------------------------------------------------------------------------------------
...
begin
   ...
   exec @w_error =  cob_cartera..sp_pagorees
   @i_banco          = @w_banco,
   @i_opcion         = 'S',          --CONSULTA SALDOS RUBROS <> K
   @i_es_normalizacion = @i_es_norm,
   ...
   and    tc_pago_cobro = 'C'
   and    tc_estado <> 'A' --CCA 436 DIFERENTES DE ANULADAS
   ...
   if @w_estado_pag = 'I' and @w_saldo_ant = @w_saldo_a_pagar begin
      print 'EXISTE ORDEN DE PAGO POR UN VALOR DE $'+convert(varchar, @w_saldo_a_pagar)
      return 2108025
   end

   --SI EXISTE EL PAGO Y ESTA INGRESADO ANULO EL PAGO
   if @w_estado_pag = 'I' begin
      exec @w_error = sp_trn_cj_reest
      @s_date         = @s_date,
      @s_user         = @s_user,
      @i_operacion    = 'R',
      @i_tramite      = @w_tramite
   ...
   print 'SE HA INGRESADO UNA ORDEN DE PAGO POR UN VALOR DE $'+convert(varchar, @w_saldo_a_pagar)
