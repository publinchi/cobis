/************************************************************************/
/*  Archivo:                grupal_xml_det.sp                           */
/*  Stored procedure:       sp_grupal_xml_det                           */
...
if exists(select 1 from sysobjects where name ='sp_grupal_xml_det')
    drop proc sp_grupal_xml_det
go
...
SELECT  TOP 1 @w_grupo=tg_grupo FROM  cob_credito..cr_tramite_grupal WHERE tg_tramite =@i_tramite
...
WHERE w_tramite = @i_tramite) AS A FOR XML EXPLICIT )
...
INSERT INTO cob_sincroniza..si_sincroniza_det (sid_secuencial,       sid_id_entidad, sid_id_1,
                                               ...
                                               sid_observacion)
       VALUES                                 (@i_secuencial, @i_inst_proc,   @i_tramite,
