/************************************************************************/
/*  Archivo:                var_monto.sp                                */
/*  Stored procedure:       sp_var_monto                                */
...
if exists (select 1 from sysobjects where name = 'sp_var_monto' and type = 'P')
   drop proc sp_var_monto
go



CREATE PROC sp_var_monto
		(@s_ssn        int         = null,
	     @s_ofi        smallint,
	     @s_user       login,
         @s_date       datetime,
...
if @w_tramite = 0 return 0
      ...
      SELECT @w_monto_tr = tr_monto--*cz_valor
      FROM cob_credito..cr_tramite--, cob_credito..cr_cotizacion
      WHERE tr_tramite = @w_tramite
      --AND tr_moneda = cz_moneda
      
      SELECT @w_valor_nuevo = convert(VARCHAR(20),@w_monto_tr)
