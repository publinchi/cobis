...
if exists (select 1 from sysobjects where name = 'sp_var_jefe_agencia' and type = 'P')
   drop proc sp_var_jefe_agencia
go



CREATE PROC sp_var_jefe_agencia
		(@s_ssn        int         = null,
	     @s_ofi        smallint,
	     @s_user       login,
         @s_date       datetime,
...
SELECT @w_sp_name='sp_var_jefe_agencia'
...
VALUES ('CARGO JEFE DE AGENCIA', 'CJAGE','I',2,'CRE')*/
...
if @w_tramite = 0 return 0
     ...
     SELECT @w_jefe_agencia = pa_int
     FROM cobis..cl_parametro
     WHERE pa_nemonico = 'CJAGE'
     AND pa_producto = 'CRE'
            
      
      SELECT @w_valor_nuevo = convert(VARCHAR(10),@w_jefe_agencia)
