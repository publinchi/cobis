/************************************************************************/
/*  Archivo:                var_calif_buro_cred.sp                      */
/*  Stored procedure:       sp_var_calif_buro_cred                      */
...
if exists (select 1 from sysobjects where name = 'sp_var_calif_buro_cred' and type = 'P')
   drop proc sp_var_calif_buro_cred
go


CREATE PROC sp_var_calif_buro_cred
		(@s_ssn        int         = null,
	     @s_ofi        smallint    = null,
	     @s_user       login       = null,
...
SELECT @w_sp_name='sp_var_calif_buro_cred'

SELECT @w_calif_buro = 'BUENO'
...
EXEC @w_return = cob_credito..sp_var_calif_buro_cred_int
@i_ente        = @w_ente,
@o_resultado   = @w_valor_nuevo OUTPUT

if @w_valor_nuevo  IS NULL
begin
...
print 'INSTANCIA: ' + convert(VARCHAR,@i_id_inst_proc) +'- CLIENTE -' + convert(VARCHAR,@w_ente) + ' CALIFICACION BURO ' + @w_valor_nuevo 
