/************************************************************************/
/*  Archivo:                         sp_notifica_apr_com_esp.sp         */
/*  Stored procedure:                sp_notifica_apr_com_esp            */
...
/************************************************************************/
/*                          PROPOSITO                                   */
/*  Notificaciones correo electronico Aprobacion Comite Especial        */
/************************************************************************/
/*                        MODIFICACIONES                                */
/*      FECHA           AUTOR           RAZON                           */
...
if exists (select 1 from sysobjects where name = 'sp_notifica_apr_com_esp')
   drop proc sp_notifica_apr_com_esp
go
CREATE PROCEDURE sp_notifica_apr_com_esp (
        @s_ssn                           int          = null,
        @s_user                          login        = null,
...
select  @w_sp_name             = 'sp_notifica_apr_com_esp',
        @w_nombre_rol          = '',
        @w_nombre_rol_com      = '',
        @w_email_oficial       = '',
        @w_email_oficial_aux   = '',
...
--Parametro Actividad
select  @w_desc_actividad = pa_char from cobis..cl_parametro where pa_producto = 'CRE' and pa_nemonico = 'AAPRCE'

--Parametro Notificaciones
select  @w_subject = pa_parametro from cobis..cl_parametro where pa_producto = 'CRE' and pa_nemonico = 'NAPRCE'

--Template
select @w_template = te_id from cobis..ns_template  where te_nombre  = 'CorreoAprobacionComiteEspecial.xslt'
     
      
--Actividad Aprobacion Comite Especial
...
begin   
   ...
   begin
        ...
        select de_oficial_inst    = (select fu_nombre from cobis..cl_funcionario where fu_funcionario = @w_id_usuario_cabecera),
   ...
   if (@w_es_todo_comite = 'S')
    begin
        ...
        insert into #tmp_destinatario
        select de_oficial_inst    = (select fu_nombre from cobis..cl_funcionario where fu_funcionario = ur_id_usuario),
...
begin    
    ...
    insert into #tmp_destinatario
    select top 1 de_oficial_inst    = (select fu_nombre from cobis..cl_funcionario where fu_funcionario = ur_id_usuario),
                 de_em_oficial_inst = (select fu_correo_electronico from cobis..cl_funcionario where fu_funcionario = ur_id_usuario),
                 de_nombre_rol      = ro_nombre_rol
    from cob_workflow..wf_rol,
         cob_workflow..wf_usuario_rol,
...
select @w_xml = '<?xml version="1.0" encoding="UTF-8"?><data><nombreRol>' + @w_nombre_rol + '</nombreRol><codigoAlterno>' + @w_codigo_alterno + '</codigoAlterno><codigoTramite>' + convert(varchar(10),@w_tramite) + '</codigoTramite></data>'
