...
/*                      MODIFICACIONES                          */
/* FECHA               AUTOR                         RAZON      */
/* 01/Abril/2019       Luis  Ramirez  	        Emision Inicial */
/****************************************************************/

USE cob_custodia
go

IF OBJECT_ID('dbo.sp_riesgos') IS NOT NULL
    DROP PROCEDURE dbo.sp_riesgos
go
create proc dbo.sp_riesgos  (
   @s_ssn                int      = null,
        ...
        begin
           ...
           begin 
              select @w_fecha_fin = op_fecha_fin,
                     ...
                     @w_riesgo = sum(am_acumulado - am_pagado)--@w_riesgo = sum(am_acumulado - am_pagado - am_exponencial)
