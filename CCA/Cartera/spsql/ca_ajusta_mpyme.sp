...
if exists (select 1 from sysobjects where name = 'sp_ajusta_mpyme')
   drop proc sp_ajusta_mpyme
go

create proc sp_ajusta_mpyme
as 
...
--tasa 8.11 cambia a 7.82
select @w_tasa1 = 7.82

--tasa 4.71 cambia a 4.61
select @w_tasa2 = 4.61
...
select @w_fecha_hoy = getdate(),
       @w_sp_name   = 'sp_ajusta_mpyme'
               
               
               
select distinct OPERACION as op, TASA_OR as porcentaje 
into #operac
from cob_cartera..ca_recalculo_mipymes_datos
...
begin
   ...
   begin
      set rowcount 0
      select * from cob_cartera..ca_recalculo_mipymes_datos order by OPERACION, DIVIDENDO 
      break
   end
   ...
   if @w_factor_ant  = 8.11 
      select @w_tasa = @w_tasa1 
   else
      select @w_tasa = @w_tasa2   
   ...
   begin tran           
   exec  @w_error = sp_recalculo_mipymes_1198
   ...
   begin
      ...
      ---PRINT 'Despues de sp_recalculo_mipymes_1198'
      ...
      update ca_recalculo_mipymes_datos set 
      CEDULA           = @w_en_ced_ruc,
      ...
      if @@error <> 0
      begin
         PRINT 'Error  EN ACTUALIZACION DE ca_recalculo_mipymes_datos'
