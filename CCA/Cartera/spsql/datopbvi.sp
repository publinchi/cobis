/************************************************************************/
/*	Archivo: 		datopbvi.sp		 		*/
/*	Stored procedure: 	sp_datos_op_bv				*/
...
if exists (select 1 from sysobjects where name = 'sp_datos_op_bv')
	drop proc sp_datos_op_bv
go

create proc sp_datos_op_bv (
        @i_banco                 cuenta      = null       
)
as
...
/*********************************/
select @w_sp_name = 'sp_datos_op_bv'
...
/** MONTO PROYECTADO **/
/**********************/
select @w_valor_a_pagar = sum((abs(am_cuota+am_gracia-am_pagado)+(am_cuota+am_gracia-am_pagado))/2)
from  ca_dividendo,ca_amortizacion
where am_operacion = @w_op_operacionca
and   di_operacion = @w_op_operacionca
and   am_operacion = di_operacion
...
@w_op_precancelacion,  @w_fecha_prox_pago,   @w_valor_a_pagar
) 
