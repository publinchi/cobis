/************************************************************************/
/*      Archivo:                rep_plaprocol_des.sp                    */
/*      Stored procedure:       sp_rep_plaprocol_des                    */
/*      Producto:               Cartera                                 */
...
/*      "Plazo promedio de colocacion en rango de fechas de desembolso  */
/************************************************************************/

use cob_cartera
go

if exists (select 1 from sysobjects where name = 'sp_rep_plaprocol_des')
   drop proc sp_rep_plaprocol_des
go

create proc sp_rep_plaprocol_des
...
create table #rep_plaprocol_des
(
tmp_oficina         smallint    not null,
tmp_nombre_ofi      varchar(24) not null,
tmp_plazo_dias_n    int         not null,
...
/* CURSOR PARA DETERMINAR TODAS LAS OPERACIONES DESEMBOLSADAS EN EL RANGO */
declare
cursor_rep_plaprocol_des cursor
for select
      op_oficina,
      ...
      sum((op_plazo * td_factor))/count(*),
      sum((op_plazo * td_factor)/30)/count(*),
      count(*)
...
open  cursor_rep_plaprocol_des
fetch cursor_rep_plaprocol_des
...
begin
    ...
    if exists (select 1 from #rep_plaprocol_des
               where tmp_oficina = @w_oficina)
    begin
        update #rep_plaprocol_des --Actualiza los renovados por el order by colocado en el cursor
    ...
    begin
        insert into #rep_plaprocol_des --Inserta primero los nuevos por el order by colocado en el cursor
...
end --end while cursor rep_plaprocol_des
close cursor_rep_plaprocol_des
deallocate cursor_rep_plaprocol_des
