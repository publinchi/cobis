...
if exists (select 1 from sysobjects where name = 'sp_rcmipymesIII')
   drop proc sp_rcmipymesIII
go

create proc sp_rcmipymesIII
as 
...
select 
@w_fecha_hoy = getdate(),
@w_sp_name   = 'sp_rcmipymesIII'
               
exec  @w_error = cob_cartera..sp_tabla_mpymeIII
if @w_error <> 0
begin
...
exec  @w_error = cob_cartera..sp_ajusta_mpymeIII
if @w_error <> 0
begin
   PRINT 'Error ' + cast (@w_error as varchar)
end
