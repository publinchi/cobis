...
if exists (select 1 from sysobjects where name = 'sp_ajusta_mpymeII')
   drop proc sp_ajusta_mpymeII
go

create proc sp_ajusta_mpymeII
as 
...
select @w_fecha_hoy = getdate(),
       @w_sp_name   = 'sp_ajusta_mpymeII'
               
               
               
select distinct OPERACION as op, TASA_OR as porcentaje 
into #operac
from cob_cartera..ca_recalculo_mipymes_datosII
...
begin
   ...
   begin
      set rowcount 0
      select * from cob_cartera..ca_recalculo_mipymes_datosII order by OPERACION, DIVIDENDO 
      break
   end
   ...
   begin tran           
   exec  @w_error = sp_recalculo_mipymesII
   ...
   begin
      ...
      update ca_recalculo_mipymes_datosII set 
      CEDULA           = @w_en_ced_ruc,
      ...
      if @@error <> 0
      begin
         PRINT 'Error  EN ACTUALIZACION DE ca_recalculo_mipymes_datosII'
