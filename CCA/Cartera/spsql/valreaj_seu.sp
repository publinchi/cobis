...
begin
   ...
   select 'Codigo'      = va_tipo,
          'Descripcion' = va_descripcion,
          'Clase'       = va_clase
   from cob_cartera..ca_valor
   where upper(va_tipo) > upper(isnull(@i_codigo,''))
   order by va_tipo ASC
...
begin
   select 'Codigo'      = va_tipo,
          ...
          'Clase'       = va_clase
      from cob_cartera..ca_valor
      where upper(va_descripcion) > upper(isnull(@i_codigo,''))
      and upper(va_tipo) like upper('%'+@i_codigo+'%')
...
begin
   ...
   select 'Codigo'      = va_tipo,
          ...
          'Clase'       = va_clase
   from cob_cartera..ca_valor
   where upper(va_tipo) > upper(@i_codigo)
     and upper(va_descripcion) like upper('%'+isnull(@i_descripcion,'')+'%')
   ORDER BY va_tipo ASC
...
begin
   select count(*) from cob_cartera..ca_valor
end
return 0
