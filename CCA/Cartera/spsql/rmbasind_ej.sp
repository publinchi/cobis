/**************************************************************************/
/*      Archivo:                        rmbasind_ej.sp                    */
/*      Stored procedure:               sp_rep_mensual_seg_bas_indiv_ej   */
...
/**************************************************************************/
/*                      PROPOSITO                                         */
/*  Reporte de facturación Mensual de seguros Básico Individual           */
/**************************************************************************/
/*                      MODIFICACIONES                                    */
/*      FECHA           AUTOR                       RAZON                 */
/*   2/AGO/2019     Edison Cajas       Estado inicial                     */
...
IF OBJECT_ID ('dbo.sp_rep_mensual_seg_bas_indiv_ej') IS NOT NULL
	DROP PROCEDURE dbo.sp_rep_mensual_seg_bas_indiv_ej
GO

create proc sp_rep_mensual_seg_bas_indiv_ej
(
...
select @w_retorno = 0,
       @w_sp_name = 'sp_rep_mensual_seg_bas_indiv_ej',
       @w_tabla =   'cob_cartera..ca_factmenssegbasindv_rep'   
   
if @t_show_version = 1
begin 
   print 'Stored Procedure: ' + @w_sp_name + ' Version:  ' + CONVERT(VARCHAR,@t_show_version)
   return 0
end
...
select 
   ...
   op_toperacion,
   (SELECT valor FROM cobis..cl_tabla a, cobis..cl_catalogo b
    WHERE a.codigo = b.tabla AND a.tabla = 'ca_toperacion' and b.codigo = op_toperacion),
   op_banco,op_cliente
  ,isnull(ente.p_p_apellido,'') + ' ' + isnull(ente.p_s_apellido,'') + ' ' + isnull(ente.en_nombre,'')
  ,(select pv_descripcion from cobis..cl_provincia where pv_provincia = (select of_provincia from cobis..cl_oficina where of_oficina = op_oficina))
  ,(select of_nombre from cobis..cl_oficina where of_oficina = op_oficial)
  ,0,0  
  ,case when op_fecha_ini >= @i_fecha_desde and op_fecha_ini <= @i_fecha_hasta 
        then op_monto 
		else 
		    (select do_saldo
                ...
                and do_fecha = (
                    select max(do_fecha)
                     ...
                     where do_banco = op_banco
                       ...
                       and year(do_fecha) = YEAR(@i_fecha_desde))) 
		end
  ,year(getdate()) - year(ente.p_fecha_nac) 
  ,ente.p_sexo,ente.p_fecha_nac 
  ,null,null,null,null,null,null
  ,op_fecha_ini,op_fecha_fin
  ,null,null,null,' '  
from ca_operacion  
left join cobis..cl_ente ente on op_cliente = en_ente
where (op_estado not in (0, 3, 6, 99) or (op_estado = 3 and op_fecha_ult_proceso between  @i_fecha_desde  and @i_fecha_hasta))
and (op_grupal = 'N' or op_grupal is null)
and op_operacion not in (select dc_operacion from ca_det_ciclo)
and op_fecha_ini <= @i_fecha_hasta
and op_operacion in (select so_operacion from ca_seguros_op where so_tipo_seguro = 'B')
...
delete cob_cartera..ca_factmenssegbasindv_rep
where secuencial >= 0

select @w_titulo = 'REPORTE DE FACTURACION MENSUAL DE SEGUROS BASICOS CREDITO INDIVIDUAL'

insert into cob_cartera..ca_factmenssegbasindv_rep values
(null,' ',' ',' ',' ',@w_titulo,' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ')

insert into cob_cartera..ca_factmenssegbasindv_rep values
(null,'TIPO PRESTAMO','PRODUCTO','CREDITO','CLIENTE','NOMBRE CLIENTE','REGIONAL','OFICINA SUCURSAL','DIAS MORA','CUOTA',
...
insert cob_cartera..ca_factmenssegbasindv_rep
select 
    secuencial
   ,tipoPrestamo
   ,producto
...
select @w_cmd = @w_parsapp + 's_app bcp -auto -login cob_cartera..ca_factmenssegbasindv_rep out '

select @w_destino = @w_path + 'factmenssegbasindv' + @w_fecha + '_' + @w_hora + '.txt'
      ,@w_errores = @w_path + 'factmenssegbasindv' + @w_fecha + '_' + @w_hora + '.err'
