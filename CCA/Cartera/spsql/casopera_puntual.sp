/************************************************************************/
/*   Archivo:              casopera_puntual.sp                          */
/*   Stored procedure:     sp_transaccion_cas_puntual                   */
/*   Base de datos:        cob_cartera                                  */
/*   Producto:             Credito y Cartera                            */
/*   Disenado por:         Elcira Pelaez Burbano                        */
/*   Fecha de escritura:   NOv.2015                                    */
/************************************************************************/
/*                               IMPORTANTE                             */
...
/************************************************************************/
/*                                PROPOSITO                             */
/*   Traslada los saldos generando una transaccion CAS. para una opera  */
/*   cion puntual ejecutado de un punto diferente al batch              */
/************************************************************************/
/*                               CAMBIOS                                */
/*   FECHA     AUTOR          CAMBIO                                    */
...
if exists (select 1 from sysobjects where name = 'sp_transaccion_cas_puntual')
   drop proc sp_transaccion_cas_puntual
go
...
create proc sp_transaccion_cas_puntual(
...
'ING',                  'CASTIGO DE OPERACIONES PUNTUAL',       @w_oficial,
'',                     '',                             'E',
@w_fecha_proceso,       0,                               getdate()
)
...
concepto    = case when dtr_concepto = 'CAP'                                                                                  then 'CAP' 
                   when dtr_concepto in ('INT', 'IMO', 'INTTRAS','IMOSEGVID','IMOSEGPRI','IMOSEGEXE','IMOSEGDAN')             then 'INT' 
                   when dtr_concepto not in ('CAP', 'INT', 'IMO', 'INTTRAS','IMOSEGVID','IMOSEGPRI','IMOSEGEXE','IMOSEGDAN')  then 'MIPYMES' 
              end,
monto       = sum(dtr_monto),
...
if @w_estado not in (@w_est_vigente, @w_est_vencido) begin
   ...
   concepto    = case when am_concepto = 'CAP'                                                                                 then 'CAP' 
                      when am_concepto in ('INT', 'IMO', 'INTTRAS','IMOSEGVID','IMOSEGPRI','IMOSEGEXE','IMOSEGDAN')            then 'INT' 
                      when am_concepto not in ('CAP', 'INT', 'IMO', 'INTTRAS','IMOSEGVID','IMOSEGPRI','IMOSEGEXE','IMOSEGDAN') then 'MIPYMES' 
                 end,
   ...
   from cob_cartera..ca_amortizacion
   where am_operacion  = @i_operacionca
   and   am_concepto  not in ('CAP','MIPYMES', 'IVAMIPYMES', 'INT', 'IMO', 'INTTRAS','IMOSEGVID','IMOSEGPRI','IMOSEGEXE','IMOSEGDAN')
...
BEGIN   
   ...
   if @w_estado_cobr = @w_estado_juridico 
   begin 
      --- CAMBIAR EL ESTADO DE LA COBRANZA OPERACION A CC-CASTIGO 
      select 
      cc_cobranza   = oc_cobranza,
      cc_banco      = @w_banco,
      cc_estado_ant = convert(varchar(10), null),
      cc_estado     = @w_estado_juridico
      into  #cambios_cobranza
      from  cob_credito..cr_operacion_cobranza
      where oc_num_operacion    = @w_banco
      ...
      cc_estado,      @s_user,               (select convert(varchar,(select fp_fecha from cobis..ba_fecha_proceso),101))
      ...
      co_estado        = 'CC',
      co_observa       = 'Cobro Castigado'
      from  #cambios_cobranza
      where co_cobranza = cc_cobranza
      if @@error <> 0  begin
         select @w_error = 710002, @o_msg = 'ERROR AL CAMBIAR EL ESTADO DE cob_credito..cr_cobranza ' 
         goto ERROR_FIN
      end      
   
      update ca_operacion set
      op_estado_cobranza = 'CC'
      where  op_operacion = @i_operacionca
      if @@error <> 0  begin
         select @w_error = 710002, @o_msg = 'ERROR AL CAMBIAR EL ESTADO DE ca_operacion ' 
         goto ERROR_FIN
      end      
   end
   ELSE
   begin
      if @w_estado_cobr in ('CI', 'CM', 'CT', 'CP')
      begin 
         ---NUEVO CODIGO
         ---CAMBIAR EL ESTADO DE LA OPERACION A CA-Cobro Administrativo 
         -- NYMR
         select 
         cc_cobranza   = oc_cobranza,
         cc_banco      = oc_num_operacion,
         cc_estado_ant = convert(varchar(10), null),
         cc_estado     = @w_estado_cobr
         into  #cambios_cobranza_otros
         from  cob_credito..cr_operacion_cobranza
         where oc_num_operacion  = @w_banco
         
         
         update #cambios_cobranza_otros set
         cc_estado_ant = co_estado
         from   cob_credito..cr_cobranza
         where  co_cobranza = cc_cobranza
         ...
         cc_cobranza,    isnull(max(ce_secuencial) + 1, 1), isnull(cc_estado_ant, 'NO'), 
         cc_estado,      @s_user,               (select convert(varchar,(select fp_fecha from cobis..ba_fecha_proceso),101))
         from  #cambios_cobranza_otros left outer join cob_credito..cr_cambio_estados on cc_cobranza = ce_cobranza
         group  by cc_cobranza, cc_estado_ant, cc_estado
...
END ---ESTADO DE LA COBRANZA HABILITADO POR PARAMTRO
