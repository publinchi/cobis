/************************************************************************/
/*   Archivo:              venproopera.sp                               */
/*   Stored procedure:     sp_cambio_estado_vencido_prorroga            */
...
/*                               CAMBIOS                                */
/*      FECHA          AUTOR            CAMBIO                          */
/*  28/dic/2021   G.Fernandez    Emision Inicial - En base al contenido */
/*                               de sp_cambio_estado_vencido            */
/*  05/sep/2022   K. Rodriguez   R193404 Ajuste det_trn por am_secuencia*/
...
if exists (select 1 from sysobjects where name = 'sp_cambio_estado_vencido_prorroga')
   drop proc sp_cambio_estado_vencido_prorroga
go

SET ANSI_NULLS ON
GO

create proc sp_cambio_estado_vencido_prorroga(
   @s_user                login,
   @s_term                varchar(30),
   @i_operacionca         int,
   @i_cotizacion          float,
...
'ING',                  'CAMBIO ESTADO A VENCIDO PRORROGA',     @w_oficial,
...
and   am_estado   <> @w_est_vencido_prorroga  --al regresar de suspenso a vencido rubros como el capital no salieron de vencido
...
dtr_dividendo    = min(am_dividendo),
dtr_concepto     = am_concepto,
dtr_estado       = @w_est_vencido_prorroga,
dtr_periodo      = am_periodo,
dtr_codvalor     = co_codigo * 1000 + @w_est_vencido_prorroga * 10 + am_periodo,
...
and   ro_tipo_rubro in ('C','I','M')
and   co_concepto  = am_concepto
group by am_concepto, am_periodo, co_codigo * 1000 + @w_est_vencido_prorroga * 10
having sum(am_acumulado - am_pagado) > 0
...
op_estado           = @w_est_vencido_prorroga
where  op_operacion = @i_operacionca
...
am_estado = @w_est_vencido_prorroga
from  ca_rubro_op
where am_operacion = @i_operacionca
and   ro_tipo_rubro in ('C','I','M')
