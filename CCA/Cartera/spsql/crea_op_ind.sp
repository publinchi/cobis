...
IF OBJECT_ID('tempdb..#operaciones') IS  NULL
...
--UPDATE cobis..ba_fecha_proceso SET fp_fecha = @i_fecha_valor
...
while @w_contador < @i_num_prestamos begin
   ...
   --select @w_tramite_0 = isnull(@w_tramite,1)
...
---CREACIÓN DE OPERACION
  ...
  -- select @w_monto  =  5000   
   ...
   select @w_fecha = @w_fecha_pro_orig
   
   select @s_ssn = @s_ssn + 1000
   
   SELECT @w_ofi_func =  @s_ofi   
   ...
   @i_moneda         = 0,
   @i_comentario     = 'Operacion creada por backend',
   @i_oficial        = @w_oficial,
   @i_fecha_ini      = @w_fecha,
   @i_monto          = @w_monto,
   ...
   @i_login          = @s_user, 
   @i_id_proceso     = 0, 
   @i_nombre_proceso ='SOLICITUD CREDITO INDIVIDUAL SANTANDER', 
   @i_campo_1        =@w_cliente, 
   @i_campo_3        =@w_tramite, 
   @i_ruteo          ='A',
   @i_id_empresa     =1,
   ...
   BEGIN
     ...
     PRINT 'ERROR: '-- + convert(VARCHAR(20),@s_date) + ' @w_inst_proceso : ' + cnvert(VARCHAR(20),@w_inst_proceso)
    
      goto ERROR
   END 
   
   select @w_operacion = op_operacion, @w_banco = op_banco, @w_forma_pago = op_forma_pago
...
--UPDATE cobis..ba_fecha_proceso SET fp_fecha = @w_fecha_pro_orig
...
if @i_mostrar_op ='S'
begin
   select  op_operacion, op_banco, en_ente, op_monto, op_cuenta
   from #operaciones, cobis..cl_ente, cob_cartera..ca_operacion
