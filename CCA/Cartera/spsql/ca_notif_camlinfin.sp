...
/************************************************************************/
/*    Archivo:                ca_notif_camlinfin.sp                     */
/*    Stored Procedure:       sp_notif_camlinfin                        */
...
/*                           MODIFICACION                               */
/*    FECHA       AUTOR         RAZON                                   */
/*  20/01/2015    Luis Moreno   CCA 479 Finagro Fase2                   */
...
if exists (select 1 from sysobjects where name = 'sp_notif_camlinfin')
   drop proc sp_notif_camlinfin
go
---JUL.30.2015
create proc sp_notif_camlinfin (
...
select 
@w_sp_name = 'sp_notif_camlinfin',
@w_subject = 'OPERACIONES CON CAMBIO DE LINEA A NO FINAGRO ' + convert(varchar(10), @w_fecha_proceso,103)
...
--BANDERA ENVIO NOTIFICACIONES FINAGRO
...
if @w_genera_notif = 'N'
begin 
  select @w_msg = 'ca_notif_camlinfin.sp --> PARAMETRO USLIFI esta PARAMETRIZADO EN N  '
  goto ERRORFIN
end

----SI NO HAY DATOS CARGADOS SIMPLEMENTE GENERE UN ERROR
if not exists ( select 1 from ca_proc_cam_linea_finagro where pc_estado     = 'P')
begin
  select @w_msg = 'ca_notif_camlinfin.sp --> NO HA DATOS PROCESADOS EN LA TABLA ca_proc_cam_linea_finagro'
  goto ERRORFIN
end
...
begin
   ...
   en_oficina_prod
   into  #procesadas
   from  cob_cartera..ca_proc_cam_linea_finagro with (nolock), 
         cobis..cl_ente with (nolock)
   where pc_fecha_proc = @w_fecha_proceso
   and   en_ced_ruc    = pc_identificacion
   and   en_tipo_ced   = pc_tipo_ide
   and   pc_estado     = 'P'
   order by pc_banco_cobis

   select @w_banco = ''
   select @w_cuerpo_correo = 'NOTIFICACION: EL PROCESO HA FINALIZADO DE MANERA EXITOSA, LOS SIGUIENTES CREDITOS FINAGRO FUERON CAMBIADOS A LA NUEVA LINEA: ' + char(10) 

   --GENERAR EL CUERPO DEL MENSAJE
   while 1=1
   begin
      select top 1 
      @w_tipo_ide       = pc_tipo_ide,
      @w_identificacion = pc_identificacion,
      @w_banco          = pc_banco_cobis,
      @w_linea_destino  = pc_linea_destino,
      @w_nombre         = en_nomlar,
   ...
   if exists (select 1 from #procesadas)
   begin
