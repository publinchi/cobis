...
if exists(select 1 from sysobjects where name = 'sp_recalculo_mipymes_1198')
   drop proc sp_recalculo_mipymes_1198
go

create proc sp_recalculo_mipymes_1198
@i_operacion            int,
@i_factor               float,
@i_factor_ant           float
...
select 
@w_sp_name        = 'sp_recalculo_mipymes_1198',
@w_valor          = 0,
@w_porcentaje     = 0,
@w_valor_asociado = 0,
...
begin
   ...
   select @w_monto_cap     = round((@w_cuota_mipyme * 1200)/(@w_periodo_int * @i_factor_ant), 0) 
