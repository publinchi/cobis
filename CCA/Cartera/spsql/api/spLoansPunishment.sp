...
if exists (select 1 from sysobjects where name = 'sp_api_loans_punishment')
   drop proc sp_api_loans_punishment
GO


CREATE PROCEDURE sp_api_loans_punishment
/************************************************************************************/
/*  Archivo:            spLoanPunishment                                            */
...
SELECT @w_sp_name = 'sp_api_loans_punishment'
...
BEGIN
   ...
   BEGIN
      ...
      --SELECT @w_es_descripcion = es_descripcion FROM ca_estado WHERE es_codigo = @w_est_castigado
      ...
      EXEC @w_error = cob_cartera..sp_cambio_estado_op_ext
           ...
           @s_ofi        =  @s_ofi,            
           @i_banco      = @i_banco,
           @i_estado_fin = @w_est_castigado --@w_es_descripcion --@w_est_castigado
     --      @t_trn        = 7135
