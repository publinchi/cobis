...
if exists (select 1 from sysobjects where name = 'sp_api_loans_change_situation')
   drop proc sp_api_loans_change_situation
GO


CREATE PROCEDURE sp_api_loans_change_situation
/************************************************************************************/
/*  Archivo:            spLoansChangeSituation                                      */
/*  Stored procedure:   sp_api_loans_change_situation                               */
...
/*  los Cambios de Estado Manuales de Operaciones de Cartera                        */
/************************************************************************************/
/*                          MODIFICACIONES                                          */
/*  FECHA             AUTOR                   RAZON                                 */
...
SELECT @w_sp_name = 'sp_api_loans_change_situation'
...
--LPO OPEN API:
   ...
   GET /loans/{loanId} ----?????????????????????????????????????????????????????????????
      --> @i_operacion = 'Q' --> sp_estados (@i_operacion='M')      
      (Consulta los estados a los que se puede cambiar la operación según el estado actual)
...
IF @i_operacion = 'S'
BEGIN
   SELECT @w_estado = es_descripcion FROM ca_operacion, ca_estado WHERE es_codigo = op_estado AND op_banco = @i_banco   
   SELECT @w_estado
...
BEGIN
   ...
   IF EXISTS (SELECT 1
              ...
              WHERE em_toperacion = @w_toperacion
                ...
                AND em_estado_fin  = @i_estado_fin) --(SELECT es_codigo FROM ca_estado WHERE es_descripcion = @i_estado_fin))
   BEGIN
