...
if exists (select 1 from sysobjects where name = 'sp_api_loans_legal')
   drop proc sp_api_loans_legal
GO


CREATE PROCEDURE sp_api_loans_legal
/************************************************************************************/
/*  Archivo:            spLoansLegal                                                */
/*  Stored procedure:   sp_api_loans_punishment                                     */
...
/*  el Cambio de estado a Castigado de Operaciones de Cartera                       */
/************************************************************************************/
/*                          MODIFICACIONES                                          */
/*  FECHA             AUTOR                   RAZON                                 */
...
SELECT @w_sp_name = 'sp_api_loans_legal'
...
--LPO OPEN API:
   ...
   POST /loans/{loanNumber} ----????????????????????????????????????????????????????????
      ...
      -->(Realiza el cambio de estado a Castigado)
...
BEGIN
   ...
   IF EXISTS (SELECT 1
              ...
              WHERE em_toperacion = @w_toperacion
                ...
                AND em_estado_fin  = @w_est_judicial)
   BEGIN
      
      --SELECT @w_es_descripcion = es_descripcion FROM ca_estado WHERE es_codigo = @w_est_judicial
      ...
      EXEC @w_error = cob_cartera..sp_cambio_estado_op_ext
           ...
           @s_ofi        =  @s_ofi,            
           @i_banco      = @i_banco,
           @i_estado_fin = @w_est_judicial --@w_es_descripcion --@w_est_judicial
          -- @t_trn        = 7135
