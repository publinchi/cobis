...
/* Consulta de descripciones de monedas en in_numlet */
/*ESTA PARTE SE COMENTA PUES TODOS LOS CHEQUES YA TIENEN PRE IMPRESO EL NOMBRE DE LA MONEDA*/
/*********************************************************************************************/
if @i_idioma = 'E'	-- Desc. de la moneda en espa¤ol 
	...
	begin
		...
		if @w_cifras = 3 
		  ...
		  if @i_idioma = 'E'	
	            ...
	            begin
			if @w_aux <> 0			
			  select @w_cent = ' ' + nl_letras_esp from in_numlet
			  where  nl_numero = @w_aux and
		          nl_tipo = 'C'
		   end
	          else
		    if @w_aux <> 0
                      select @w_cent = ' ' + nl_letras_otro from in_numlet
		      where  nl_numero = @w_aux and
		      nl_tipo = 'C'
		...
		if @w_cifras = 2 
		 begin
		  ...
		  if @w_aux = 1 
		   begin
    			   if @i_idioma = 'E'	
				select @w_dec = ' ' + nl_letras_esp from in_numlet
				where  nl_numero = convert(tinyint, @w_numero)
				and    nl_tipo = 'U'
			   if @i_idioma = 'O' or @i_idioma = 'I'
				select @w_dec = ' ' + nl_letras_otro from in_numlet
				where  nl_numero = convert(tinyint, @w_numero)
				and    nl_tipo = 'U'
		...
		if @w_cifras = 1 
		 begin
		  ...
		  if @w_aux <> 0 
    	 	     if @i_idioma = 'E'	
			  select @w_unid = ' ' + nl_letras_esp from in_numlet
		          where  nl_numero = @w_aux
			  and nl_tipo = 'U'
    	 	     if @i_idioma = 'O'	or @i_idioma = 'I'
			  select @w_unid = ' ' + nl_letras_otro from in_numlet
		          where  nl_numero = @w_aux
			  and nl_tipo = 'U'
		 end 
	end	
