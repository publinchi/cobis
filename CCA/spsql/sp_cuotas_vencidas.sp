/************************************************************************/
/*   NOMBRE LOGICO:      sp_cuotas_vencidas.sp                          */            
/*   NOMBRE FISICO:      sp_cuotas_vencidas                             */
...
/*  préstamos vencidas para recordarles de hacer su pago.               */
/************************************************************************/
/*                        MODIFICACIONES                                */
/*  FECHA          AUTOR             RAZON                              */
...
/*  05/Dic/2023    Kevin Rodríguez   R220843 Fecha y horario envio notif*/
/*  12/Dic/2023    Kevin Rodríguez   R220847 Modificación cuerpo mensaje*/
/************************************************************************/

use cob_cartera
go

if exists (select 1 from sysobjects where name = 'sp_cuotas_vencidas' and type = 'P')
    drop procedure sp_cuotas_vencidas
go
...
if exists (select 1 from sysobjects where name = '#cuotas_vencidas')
   drop table #cuotas_vencidas
...
select @w_fecha_hasta = dateadd(day, @w_restar_dias, @w_fecha_proceso)

if @w_valida_feriados = 'N'
begin
   select @w_fecha_desde = dateadd(day, @w_restar_dias, @w_fecha_proceso)
end
else
begin
   while @w_end_while = 'N'
   begin
      ...
      if @w_fecha_habil = @w_fecha_proceso
         select @w_restar_dias = @w_restar_dias - 1
      else
         select @w_end_while = 'S'
   end
end

insert into #cuotas_vencidas
select ea_ente, 
       op_banco,
       ea_telef_recados
from ca_operacion,
...
declare cur_cuotas_vencidas cursor
for select 
       cv_cod_cliente,
       cv_num_operacion,
       cv_num_celular
    ...
    fetch next from cur_cuotas_vencidas into
       @w_num_cliente,
       @w_num_operacion,
       @w_num_cel
    ...
    begin
       ...
       select @w_contenido_msg = 'Estimado(a) cliente, ENLACE le recuerda realizar el pago vencido de su crédito ' + @w_num_operacion + 
                                 '. Si Ud. ya realizó su pago, hacer caso omiso a este mensaje.'
       ...
       if @w_num_cel is not null and @w_num_cel != ''
		begin
		   ...
		   @i_prioridad       = 1,
		   @i_to              = @w_num_cel, 
		   @i_subject         = 'NOTIFICACIÓN DE CUOTAS VENCIDAS', 
