/************************************************************************/
/*	Archivo:		carinfba.sp				*/
/*	Stored procedure:	sp_carga_inf_bancoldex   	        */
...
/************************************************************************/
/*				PROPOSITO				*/
/*	Genera informacion para conciliacion BANCOLDEX                  */
/************************************************************************/
/*                              CAMBIOS                                 */
/*      FECHA              AUTOR             CAMBIOS                    */
/************************************************************************/
...
if exists (select 1 from sysobjects where name = 'sp_carga_inf_bancoldex')
   drop proc sp_carga_inf_bancoldex
go

create proc sp_carga_inf_bancoldex
...
select 
@w_sp_name          = 'sp_carga_inf_bancoldex'
...
select @w_bancoldex = pa_char
from   cobis..cl_parametro
where  pa_producto = 'CCA'
and    pa_nemonico = 'BCOLDE'
set transaction isolation level read uncommitted
...
truncate table ca_plano_dia_bancoldex
...
and   op_tipo_linea = @w_bancoldex  ---SOLO LAS PASIVAS DE BANCOLDEX
...
begin   
   ...
   begin
      	insert into ca_conci_dia_bancoldex(
      	cb_fecha_proceso, 	cb_linea,		cb_num_oper_cobis, 
	...
	cb_valor_mora,		cb_neto_pagar,		cb_marcar_diff, 
	cb_no_conciliada        
        )                        
	values(
	@w_fecha_proceso,	@w_op_tipo_linea,	@w_op_banco, 
   	@w_op_codigo_externo,	@w_ciudad_ofi, 		@w_op_nombre,	
	@w_op_ref_exterior,	@w_saldo_capital,	@w_tasa_nominal,		
	@w_di_dias_cuota, 	@w_abono_interes,	@w_abono_capital,		
	@w_valor_imo,   	@w_valor_neto,		null,
  	null
        )
        if @@error != 0
           PRINT 'error al insertar en ca_conci_dia_bancoldex ' + @w_op_banco
   end

   ---select * from ca_conci_dia_bancoldex
