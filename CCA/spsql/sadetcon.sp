/************************************************************************/
/*		Nombre Fisico:			sadetcon.sp								*/
/*		Nombre Logico:			sp_saldos_det_conta						*/
...
if exists (select 1 from sysobjects where name = 'sp_saldos_det_conta')
   drop proc sp_saldos_det_conta
go

create proc sp_saldos_det_conta
@i_filial		smallint = 1,
...
select @w_sp_name = 'sp_saldos_det_conta'

select @w_est_novigente = 0,
@w_est_cancelado = 3

/** LIMPIAR TABLA TEMPORAL **/
delete ca_saldos_contables_tmp
where   sc_fecha_proceso = @i_fecha
...
while @@fetch_status = 0 begin
   ...
   select am_concepto, am_estado, (abs(sum(am_acumulado + am_gracia - am_pagado))+ sum(am_acumulado + am_gracia - am_pagado))/2  
   from   ca_amortizacion
   where  am_operacion = @w_op_operacion
   and    (
	    (am_estado != 3 and am_concepto in ('CAP','IMO')) or
	    (am_estado not in (0,3) and am_concepto like 'SVI%') or
	    (am_estado not in (0,3) and am_concepto like 'INT') 
   	  )
   group by am_concepto, am_estado
   order by am_concepto, am_estado
   for read only

   open cursor_amortizacion
   
   fetch cursor_amortizacion into
   @w_am_concepto, @w_am_estado, @w_saldo
   ...
   while @@fetch_status = 0 begin
	...
	@w_dp_cuenta = '',
      ...
      and   dp_tipo_tran   = 'S'   -- Parchado Para Generar Datos JCQ 01/03/2003
      for read only
      ...
      if @i_debug = 'S' print 'CONCEPTO--> CUENTA-->' + @w_am_concepto + @w_dp_cuenta
      ...
      while 0 = 0 begin
	    ...
	    if charindex('CESI'  ,@w_trama)=1 select @w_clave=@w_op_clase + "." + convert(varchar(10),@w_am_estado)
            ...
            if @w_clave != ""  begin 
               ...
               if @i_debug = 'S' 
               print 'TRAMA--> %1! CLAVE-->  RESULTADO--> ' + @w_trama + @w_clave +@w_resultado
