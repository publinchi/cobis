...
if exists (select 1 from sysobjects where name = 'sp_tabla_mpymeII')
   drop proc sp_tabla_mpymeII
go

create proc sp_tabla_mpymeII
as 
  if exists (select 1 from sysobjects where name = 'ca_recalculo_mipymes_datos_369' )
     drop table ca_recalculo_mipymes_datos_369 
  ...
  into ca_recalculo_mipymes_datos_369
  ...
  and   op_monto_aprobado >= 2577400  
  and   ro_concepto    = am_concepto
  and   ro_porcentaje  = 7.82
