...
if exists (select 1 from sysobjects where name = 'sp_operacion_tmp')
...
/************************************************************************/
/*  NOMBRE LOGICO:        opertmp.sp                                    */
/*  NOMBRE FISICO:        sp_operacion_tmp                              */
...
/************************************************************************/
/*                              PROPOSITO                               */
/*      Transacciones de mantenimiento de una operacion temporal        */
/************************************************************************/
/*                              CAMBIOS                                 */
/*      NOV-23-20006     E.Pelaez         def 7497                      */
/*      ENE-05-2017      L.Regalado       Incluir concepto de Agrupa-   */
/*                                        miento de Operaciones         */
/*      MAY-05-25        Jorge Salazar    CGS-S112643                   */
/*      MAR-07-2019      Adriana Giler    CCA-S226083-Campos de Grupales*/
/*      ABR-16-2019      Adriana Giler    Inclusion Regla Dia Pago y Plz*/
/*      JUL-01-2019      Adriana Giler    Ajustes Originador            */
/*      JUL-03-2019      Adriana Giler    Obtener Cuenta ahorro Grupal  */
/*      ENE-02-2020      Luis Ponce       Manejo dia fijo-varios tplazo */
/*      DEC-08-2020      Patricio Narvaez Reestructuracion              */
/* ENE/06/2021   P.Narvaez  Tipo de Reestructuracion/Tipo Renovacion    */
/*      APR-19-2021      Aldair Fortiche  Se inicia por defecto en null,*/
/*										  y se establece campo 			*/
/*										  admin_individual en N,		*/
/*										  unicamente si viene en Null	*/
/*										  								*/
/*      SEP-16_2021	     A. Monroy		  El campo op_reestructuracion  */
/*										  "N" por default.				*/
/*      Ene-06-2022      G Fernandez      Ingreso de campo para         */
/*                                        grupo contable                */
/*      Abr-06-2023      G Fernandez      S785442 Ingreso de nuevo campo*/
/*                                        oda_categoria_plazo           */
/*      Jun-07-2023      K. Rodr�guez     S809862 Colum Doc. tributario */
/*      Sep-30-2024      A. Quishpe       R244659:Se añade nolock       */
/************************************************************************/
...
--PQU pasar ahorro esperado y esquema de creaci�n 
--LPO TEC Entonces se pasan los dos campos nuevos de ca_operacion (op_admin_individual y op_estado_hijas)
   @i_admin_individual          CHAR(1)       = null,  --LRE 28Ago19 Se setea siempre admin grupal -- Modificado: AFL-19APR2021
   @i_estado_hijas              CHAR(1)       = NULL,  --LPO TEC
   ...
   @i_grupo_contable    catalogo      = null  --GFP Ene-06-2022
  ...
  @w_tipo_amortizacion    varchar(10),
   ...
   @w_cuenta_grupo              varchar(24),    --AGI 03JUL19    
   @w_monto_base                MONEY,           --AGI 03JUL19    
...
begin
   ...
   begin
      if not exists( select 1 from ca_operacion where op_banco = @i_anterior) 
         return 710074 
   end
...
/*
if @i_grupal = 'N'  --AGI Reglas solo aplica a individuales
begin
    ...
    IF ((convert(INT,@w_valIni) > @i_dia_fijo OR convert(INT,@w_valFin) < @i_dia_fijo)) AND @i_grupal <> 'S' --LPO TEC Para que NO se eval�e la regla de Dia de Pago para Grupales.
    AND @i_dia_fijo  > 0 --LPO TEC Manejo dia fijo-varios tplazo (SVA)
        return  2110105

    --REGLA DE PLAZOS'
    if @i_tplazo not in('W','Q' )
    begin
		declare @w_td_factor int
		select @w_td_factor = isnull(td_factor, 0) from cob_cartera..ca_tdividendo where  td_tdividendo = @i_tplazo
        select  @w_plazo_meses = (isnull(@i_plazo, 0) * @w_td_factor)/30.0
...
--LPO mientras se sube parametrizaci�n de Reglas a la nube:
        ...
        Begin
            ...
            if not exists(SELECT number FROM cob_pac..intlist_to_tbl(@w_return_results_plazo,',') WHERE number = @w_plazo_meses) AND @i_grupal <> 'S' --LPO TEC Para que NO se eval�e la Regla de Plazos en grupales.
            BEGIN
...
--LPO FIN mientras se sube parametrizaci�n de Reglas a la nube        
    end
    else
    begin    
...
--GFP Se cometa por que no hay modulo de cob_ahorros para ENLACE
/*
--AGI 03JUL19 - Obtener cuenta grupal

If @i_grupal = 'S' AND @i_admin_individual = 'N'  --PQU integraci�n agregu� el tipo de administracion
begin
    execute @w_return = cob_ahorros..sp_consulta_cuenta_grupo
            @s_ssn          = @s_ssn,
            @s_user         = @s_user,
            @s_sesn         = @s_sesn,
            @s_term         = @s_term,
            ...
            @i_grupo        = @i_grupo,
            @i_reactiva_cta = 'N',
            @o_cta_grupo    = @w_cuenta_grupo out ,
            @o_ahorro_base  = @w_monto_base out 
            
    if @w_return != 0 return @w_return 
    
    select @i_cuenta = @w_cuenta_grupo
...
begin
   ...
   IF @w_dt_naturaleza = 'P' --Operaciones Pasivas
   BEGIN
      
      INSERT INTO ca_datos_adicionales_pasivas_t (
      dat_operacion,     dat_tipo_acreedor,    dat_linea,      dat_numreg_bc,        dat_num_cont,    
      dat_tipo_deuda,    dat_fecha_aut,        dat_num_aut,    dat_num_facilidad,    dat_forma_reposicion,
      dat_causa_fin_sub, dat_mercado_obj_fin
      )
      VALUES
      (@i_operacionca,   @i_tipo_acreedor,     @i_lin_credito, @i_numreg_bc,         @i_num_cont,
       @i_tipo_deuda,    @i_fecha_aut,         @i_num_aut,     @i_num_facilidad,     @i_forma_reposicion,
       @i_causa_fin_sub, @i_mercado_obj_fin
      )
...
--PQU grabar los 2 nuevos campos
--LPO TEC Entonces se graban los 2 campos nuevos
   opt_admin_individual,     opt_estado_hijas,              opt_tipo_renovacion,
   opt_tipo_reest
   )
   values (
   ...
   isnull(@i_reestructuracion,'N'),   @i_subtipo,                   @i_naturaleza, -- AMO 20210916 op_reestructuracion por defecto = 'N'
   ...
   @i_grupal,                         @i_fondeador,                 isnull(@i_admin_individual, 'N'), --LPO TEC -- Modificado: AFL-19APR2021
   @i_estado_hijas,                   @i_tipo_renovacion,           @i_tipo_reest
   )
...
--     PRINT 'opertmp.sp error de insercion de registro ' + cast(@@error as varchar)
   ...
   odt_operacion,       odt_estado_gestion_cobranza,  odt_aceptar_pagos, 
   odt_grupo_contable,  odt_categoria_plazo,          odt_tipo_documento_fiscal)
   values (
   @i_operacionca,      null,                         null, 
   @i_grupo_contable,   null,                         null)
  ...
  --LRE 05/Ene/2016 Insertar Operacion con concepto de Agrupamiento en caso de que aplique
    ...
    begin
       --print 'Llamar a proceso que inserta en tabla ca_operacion_ext_tmp'
       exec @w_return = sp_operacion_param
            @i_operacion = "I",
...
begin
   ...
   begin
      update ca_operacion_tmp with (rowlock) set
      ...
      --LRE 04Sep opt_sector                     = isnull(@i_sector,opt_sector),
      opt_sector                     = opt_sector,
      ...
      opt_precancelacion             = isnull(@i_precancelacion,opt_precancelacion),
      ...
      opt_grupal                     = isnull(@i_grupal, opt_grupal),
      opt_fondeador                  = isnull(@i_fondeador, opt_fondeador),
      opt_tipo_renovacion            = isnull(@i_tipo_renovacion, opt_tipo_renovacion),
      opt_tipo_reest                 = isnull(@i_tipo_reest, opt_tipo_reest)
      --PQU actualizar solo el ahorro esperado
      --LPO TEC No aplica ahorro esperado y no se debe actualizar aqu� el op_admin_individual ni el op_estado_hijas
      where opt_operacion            = @i_operacionca

      if @@error != 0 
        return 710002
	  
	  --GFP Ene-06-2022 Actualizacion en tabla de datos adicionales
	  update ca_operacion_datos_adicionales_tmp set
	  odt_grupo_contable             = isnull(@i_grupo_contable,odt_grupo_contable)
      ...
      if @i_subtipo_linea = @w_param_microcred
      begin
           --print 'Llamar a proceso que actualiza en tabla ca_operacion_ext_tmp desde actualizacion'
    ...
    begin
     ...
     --- HEREDAR PARAMETROS DE OPERACION FACTORING PADRE 
      ...
      /* ACTUALIZACION DE RUBROS INTERES Y MORA EN CA_RUBRO_OP_TMP DE HIJA */

      delete from ca_rubro_op_tmp
   ...
   BEGIN
      ...
      UPDATE ca_datos_adicionales_pasivas_t
      SET dat_forma_reposicion = @i_forma_reposicion,
          dat_num_facilidad    = @i_num_facilidad,
          dat_num_aut          = @i_num_aut,
          dat_fecha_aut        = @i_fecha_aut,
          dat_tipo_deuda       = @i_tipo_deuda,
          dat_num_cont         = @i_num_cont,
          dat_numreg_bc        = @i_numreg_bc,
          dat_linea            = @i_lin_credito,
          dat_tipo_acreedor    = @i_tipo_acreedor,
          dat_mercado_obj_fin  = @i_mercado_obj_fin,
          dat_causa_fin_sub    = @i_causa_fin_sub
      WHERE dat_operacion   = @i_operacionca      
