/**********************************************************************/
/*  Archivo           :   cli_cr_act_docs.sp                          */
/*  Stored procedure  :   sp_cli_cr_act_docs                          */
...
if exists (select 1
           from   sysobjects
           where  name = 'sp_cli_cr_act_docs')
           drop   proc sp_cli_cr_act_docs
go

create proc sp_cli_cr_act_docs
(
  @s_ssn          int          = null,
  @s_user         login        = null,
...
select @w_sp_name = 'sp_cli_cr_act_docs'
...
update cob_credito..cr_deudores
   set de_ced_ruc =  @i_en_ced_ruc
 where de_cliente =  @i_ente
   and de_tramite not in (select tg_tramite 
                            from #operaciones_grupales)
