/************************************************************************/
/*  Archivo:                referencias_bancaria_int.sp                 */
/*  Stored procedure:       sp_referencias_bancaria_int                 */
/*  Producto:               Clientes                                    */
/*  Disenado por:           Bruno Duenas                                */
/*  Fecha de escritura:     13-09-2021                                  */
/************************************************************************/
/*                              IMPORTANTE                              */
...
/*   en el servicio rest del sp_ref_eco                                 */
/************************************************************************/
/*                        MODIFICACIONES                                */
/*  FECHA       AUTOR           RAZON                                   */
/*  13-09-2021  BDU             Emision inicial                         */
/************************************************************************/
...
GO
if exists (select 1 from sysobjects where name = 'sp_referencias_bancaria_int')
   drop proc sp_referencias_bancaria_int
go

create proc sp_referencias_bancaria_int (
...
select
@w_sp_name          = 'cob_interface..sp_referencias_bancaria_int',
@w_error            = 1720548,
@w_lista_obs        = '0-9\a-zA-Z\Á\á\é\í\ó\ú\É\Í\Ó\Ú\Ñ\ñ\,\.\)\_\'+char(39)+'\---\"\&\#\!\¡\(\)\¿\?\ ',
@w_lista_cta        = '0-9\a-zA-Z\---\_\ ',
@w_long_obs         = 64
...
begin
   ...
   if @i_operacion = 'I' and isnull(@i_tipo,'') = '' 
   begin
      select @w_valor_campo  = 'referenceType'
      goto VALIDAR_ERROR   
   end
   
   if isnull(@i_referencia,'') = '' and @i_operacion = 'U' and @i_referencia <> 0
   begin
   ...
   if isnull(@i_cuenta,'') = '' 
   begin
      select @w_valor_campo  = 'accountNumber'
      goto VALIDAR_ERROR   
   end
   ...
   if isnull(@i_fecha_apertura,'') = '' 
   begin
      select @w_valor_campo  = 'openingDate'
      goto VALIDAR_ERROR   
   end
   
   if @i_tipo = 'B' and isnull(@i_tipo_cta,'') = '' 
   begin
      select @w_valor_campo  = 'accountType'
      goto VALIDAR_ERROR   
   end
   
   if @i_tipo = 'M' and isnull(@i_estado_mon,'') = '' 
   begin
      select @w_valor_campo  = 'walletStatus'
      goto VALIDAR_ERROR   
   end
...
/* VALIDAR QUE EL CLIENTE EXISTA */
   ...
   /* VALIDAR REFERENCIA */
   if @i_operacion = 'U' and not exists(select 1 from cobis..cl_economica where ente = @i_ente and referencia = @i_referencia)
   begin
      select @w_error = 1720514
      goto ERROR_FIN   
   end
   
   /* VALIDAR TIPO DE REFERENCIA */
   if @i_operacion = 'I' and (@i_tipo not in ('B', 'M'))
   begin
   ...
   if(@i_fecha_apertura > (select  fp_fecha  from cobis..ba_fecha_proceso))
   begin
   ...
   if(@i_operacion = 'I')
   begin
      if not exists(select 1 from cob_bancos..ba_banco where ba_tipo = @i_tipo and ba_codigo = @i_banco)
      begin
   ...
   begin
      if not exists(select 1 from cob_bancos..ba_banco where ba_tipo = (select tipo from cobis..cl_economica where ente = @i_ente and referencia = @i_referencia) and ba_codigo = @i_banco)
      begin
         select @w_error = 1720560 
         goto ERROR_FIN
      end
   ...
   exec @w_error = cobis..sp_validar_catalogo  @i_tabla = 'ba_tcuenta', @i_valor = @i_tipo_cta       
   if @w_error <> 0 and @w_error != 1720018 goto ERROR_FIN 
   else if @w_error = 1720018 
   begin 
      select @w_valor_campo  = @i_tipo_cta         
      select @w_error = 1720552 
      goto VALIDAR_ERROR 
   end
   if(@i_tipo = 'M')
   begin
      exec @w_error = cobis..sp_validar_catalogo  @i_tabla = 'cl_estado_monedero', @i_valor = @i_estado_mon         
      if @w_error <> 0 and @w_error != 1720018 goto ERROR_FIN 
      else if @w_error = 1720018 
      begin 
         select @w_valor_campo  = @i_estado_mon         
         select @w_error = 1720552 
         goto VALIDAR_ERROR 
      end
   end
   ...
   /* CUENTA */
   select @w_caracter = cob_interface.dbo.fn_valida_caracter(@i_cuenta, @w_lista_cta)
   if(@w_caracter is not null)
   begin
      select @w_error = 1720562
      set @w_valor_campo = @w_caracter +' - accountNumber'
      goto VALIDAR_ERROR
   end
...
begin
   ...
   /* VALIDAR REFERENCIA */
   if not exists(select 1 from cobis..cl_economica where ente = @i_ente and referencia = @i_referencia)
   begin
      select @w_error = 1720514
      goto ERROR_FIN   
   end
